...
document.querySelectorAll(".project").forEach(el=>el.addEventListener("click",()=>{if(el.dataset.video==="drive"){const src=el.dataset.src;if(src)window.open(src,"_blank","noopener,noreferrer");return;}if(el.classList.contains("graphic-project")){const img=el.querySelector("img");if(img)window.open(img.src,"_blank","noopener,noreferrer");return;}const id=el.dataset.id,start=el.dataset.start||0;frame.src=`https://www.youtube-nocookie.com/embed/${id}?autoplay=1&rel=0&modestbranding=1&playsinline=1&start=${start}`;player.classList.add("open");player.setAttribute("aria-hidden","false");document.body.style.overflow="hidden";}));
