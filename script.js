const graphicDesign = [
  {title:"India's Most Brutal Serial Killer", src:"assets/graphic-01.png"},
  {title:"A’ja Wilson — WNBA Champion", src:"assets/graphic-02.jpg"},
  {title:"Virat Kohli — 100 v/s Pakistan", src:"assets/graphic-03.jpg"},
  {title:"Hypocrite — Samay Raina / Ashneer", src:"assets/graphic-04.png"}
];
...
function graphicCard(item,index){const n=String(index+1).padStart(2,"0");return `<article class="project graphic-project"><div class="project-media"><img src="${item.src}" alt="${item.title}" loading="lazy"><span class="num">${n}</span><span class="tag">GRAPHIC DESIGN</span></div><div class="project-meta"><h3>${item.title}</h3><p>DESIGN → VIEW</p></div></article>`;}
...
document.querySelectorAll(".project").forEach(el=>el.addEventListener("click",()=>{if(el.dataset.video==="drive"){const src=el.dataset.src;if(src)window.open(src,"_blank","noopener,noreferrer");return;}const id=el.dataset.id,start=el.dataset.start||0;frame.src=`https://www.youtube-nocookie.com/embed/${id}?autoplay=1&rel=0&modestbranding=1&playsinline=1&start=${start}`;player.classList.add("open");player.setAttribute("aria-hidden","false");document.body.style.overflow="hidden";}));
document.getElementById("playerClose").addEventListener("click",close);player.addEventListener("click",e=>{if(e.target===player)close()});document.addEventListener("keydown",e=>{if(e.key==="Escape")close()});
